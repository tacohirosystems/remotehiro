-- Deploy remotehiro:20260123102636_add_business_regions to sqlite

BEGIN IMMEDIATE;

CREATE TABLE business_regions(
  business_region_id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL,
  iso2 TEXT NOT NULL CHECK(length(iso2) = 2),
  emoji TEXT
) STRICT;

INSERT INTO business_regions(business_region_id, name, iso2, emoji) VALUES (1, 'European Union', 'EU', '🇪🇺');
ALTER TABLE countries ADD COLUMN business_region_id INTEGER REFERENCES business_regions(business_region_id);

UPDATE countries
SET business_region_id = 1
WHERE
  country_id = 15
  OR country_id = 22
  OR country_id = 34
...
ALTER TABLE jobs_applicant_locations ADD COLUMN business_region_id INTEGER REFERENCES business_regions(business_region_id);
ALTER TABLE jobs_location_salaries ADD COLUMN business_region_id INTEGER REFERENCES business_regions(business_region_id);
