-- Deploy remotehiro:20260115235814_redo_tables to sqlite
...
-- 2.0: jobs
INSERT INTO temp_jobs (job_id, company_id, employment_type_id, category_id, is_remote, position, apply_email, apply_url, post_url, description, addon_pinned_until, addon_basic_highlight, created_at, updated_at, bumped_at, verified_at)
SELECT job_id, company_id, employment_type_id, category_id, is_remote, position, apply_email, apply_url, post_url, description, addon_pinned_until, addon_basic_highlight, created_at, updated_at, bumped_at, verified_at
FROM jobs;

-- 2.1: jobs_applicant_locations
INSERT INTO temp_jobs_applicant_locations(job_id, region_id, subregion_id, country_id, state_id, city_id)
SELECT job_id, region_id, subregion_id, jal.country_id, c.state_id, jal.city_id
FROM jobs_applicant_locations jal
LEFT JOIN cities c
  ON jal.city_id = c.city_id;

-- 2.2: jobs_locations
INSERT INTO temp_jobs_locations(job_id, region_id, subregion_id, country_id, state_id, city_id)
SELECT job_id, region_id, subregion_id, jl.country_id, c.state_id, jl.city_id
FROM jobs_locations jl
LEFT JOIN cities c
  ON jl.city_id = c.city_id;

-- 2.3
INSERT INTO temp_jobs_location_salaries(job_id, region_id, subregion_id, country_id, state_id, city_id, min_salary, max_salary, currency_id, created_at)
SELECT job_id, region_id, subregion_id, jls.country_id, c.state_id, jls.city_id, min_salary, max_salary, currency_id, jls.created_at
FROM jobs_location_salaries jls
LEFT JOIN cities c
  ON jls.city_id = c.city_id;

-- 3. Get rid of tables that reference `jobs` which is A LOOOT OF THEM.
